...
def _print_validation_results(results: list[UrlCheckResult], status_filter) -> None:
    ...
    if status_filter:
        print(
            f"Displayed {len(filtered_results)} result(s) after --status {status_filter} filter"
        )


def _run_reference_export(
    config: PipelineConfig, check_results: list[UrlCheckResult]
) -> None:
    ...
    ris_count, bib_count, failed_count, resolve_failed_count, output_folder = (
        export_reference_files(
            pdf_path=config.pdf,
    ...
    print(
        f"Exported {ris_count} RIS and {bib_count} BibTeX file(s) to {output_folder}{suffix}"
    )
...
def _resolve_report_path(
    config: PipelineConfig, export_folder: Path | None
) -> Path | None:
...
def run_pipeline(config: PipelineConfig) -> None:
    ...
    if _should_validate(config):
        ...
        check_results = validate_urls(
            urls, timeout_seconds=config.timeout, workers=config.workers
        )
