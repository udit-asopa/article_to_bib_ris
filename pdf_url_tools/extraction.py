...
from .utils import clean_url, dedupe_preserve_order, drop_truncated_prefix_urls, is_doi_url, should_continue_after_whitespace
...
def extract_doi_urls_from_text(text: str) -> list[str]:
    ...
    for prefix_match in DOI_PREFIX_PATTERN.finditer(text):
        candidate = _scan_url_from_prefix(text, prefix_match.start(), prefix_match.end())
