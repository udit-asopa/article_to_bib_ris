...
def _request_url(url: str, timeout_seconds: float, method: str) -> tuple[bool, str, tuple[str, ...]] | None:
    ...
    try:
        ...
        with urlopen(request, timeout=timeout_seconds) as response:
            ...
            if status is None or status < 400:
                return True, f"reachable ({status})" if status is not None else "reachable", redirects
...
def check_url_reachable(url: str, timeout_seconds: float) -> tuple[bool, str, tuple[str, ...]]:
...
def validate_single_url(url: str, timeout_seconds: float) -> UrlCheckResult:
    ...
    is_reachable, reason, redirected_links = check_url_reachable(url=url, timeout_seconds=timeout_seconds)
    return UrlCheckResult(url=url, is_valid=is_reachable, reason=reason, redirected_links=redirected_links)
